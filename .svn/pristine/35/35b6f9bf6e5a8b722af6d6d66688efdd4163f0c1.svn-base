...
<html>
	...
	<body>
		<div class="admin-main">
			...
			<blockquote class="layui-elem-quote">
		       ...
		       <a href="javascript:void(0)" class="layui-btn layui-btn-small" id="search">
				 ...
				 <a href="${request.contextPath}/backpage/admin/admin_to_input" class="layui-btn layui-btn-small" >
		...
		<script>
			...
			layui.use(['icheck', 'laypage','layer'], function() {
				...
				laypage({
					...
					jump: function(obj, first) {
						...
						if(!first) {
						 $.ajax({
				           url:'.${request.contextPath}/backpage/admin/getlist',
				           type:'post',
						 ...
						 $.ajax({
				           ...
				           success:function(data) { 
                            ...
                            $.each(data.list,function(i,val){
								...
								str += '<td>'
								str += '<a href="${request.contextPath}/backpage/admin/input?id='+val.id+'"  class="layui-btn layui-btn-mini">编辑</a>'
								str += '<a href="javascript:del('+val.id+');" data-id="1" data-opt="del" class="layui-btn layui-btn-danger layui-btn-mini" id='+ val.id+'>删除</a>'
                                str += '</tr>'
			...
			function del(id){
			$.ajax({
				           url:'${request.contextPath}/backpage/admin/admin_del',
				           type:'post',
				           data:{"id":id},
				           success:function(data) { 
				              layer.msg("删除成功",{time:2000});
				              setTimeout(function(){window.location.href="${request.contextPath}/backpage/admin/admin_list";},1000);
